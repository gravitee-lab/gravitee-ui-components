...
export class GvPolicyStudio extends KeyboardElement(LitElement) {
  ...
  _onSchemaFormChange (event) {
    if (!this._registeredSchemaForms.includes(event.originalTarget)) {
      this._registeredSchemaForms.push(event.originalTarget);
    }
  }
  ...
  _onSaveAll () {
    const confirmForms = this._registeredSchemaForms.filter((form) => form && form.dirty).map((form) => form.confirm());
  ...
  render () {
    return html`<div class="box">
        ...
        <gv-tabs .value="${this.tabId}" .options="${this._tabs}" @gv-tabs:change="${this._onChangeTab}">
            ${this._renderDesign()}
